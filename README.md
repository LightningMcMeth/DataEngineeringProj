...
5. Конект `mysql_game_db` підставляється автоматично через `docker-compose.yml`, тому вручну в UI його створювати не потрібно.
6. В UI знайти DAG-и `game_analytics_hourly` і `game_analytics_daily`, unpause і за потреби натиснути `Trigger DAG`.
   Для повного ELT-циклу спочатку запускай `game_analytics_hourly`; `game_analytics_daily` перебудовує важкі mart-агрегації окремо.
...
- [x] Airflow Connection `mysql_game_db` (підставляється автоматично через env `AIRFLOW_CONN_MYSQL_GAME_DB`)
...
- [x] Hourly DAG `game_analytics_hourly` (файл `dags/game_analytics_pipeline_dag.py`)
- [x] Daily DAG `game_analytics_daily` (файл `dags/game_analytics_daily_dag.py`)
